{{-- resources/views/book_formats/index.blade.php --}}
@extends('layouts.app')
@section('title', 'Book Formats')
...
<div class="offcanvas offcanvas-end" tabindex="-1" id="bfCanvas" style="width:420px;">
    <div class="offcanvas-header border-bottom py-3">
        <div>
            <h5 class="offcanvas-title fw-bold mb-0">
                <i id="canvasIcon" class="feather-plus-circle me-2 text-primary"></i>
                <span id="canvasTitle">Add Book Format</span>
            </h5>
            <small class="text-muted">Used for classifying how a book is sold.</small>
        </div>
    ...
    <div class="offcanvas-body">
        <form id="bfForm" novalidate>
            @csrf
            <input type="hidden" id="editBfId">

            <div class="mb-3">
                <label class="form-label fw-semibold">Format Code <span class="text-danger">*</span></label>
                <input type="text" id="bf_code" class="form-control text-uppercase" maxlength="20" placeholder="e.g. EBOOK, AUDIOBOOK, PHYSICAL">
                <div class="field-error" id="codeError"></div>
            </div>

            <div class="mb-3">
                <label class="form-label fw-semibold">Format Name <span class="text-danger">*</span></label>
                <input type="text" id="bf_name" class="form-control" placeholder="e.g. eBook">
                <div class="field-error" id="nameError"></div>
            </div>

            <div class="mb-3">
                <label class="form-label fw-semibold">Icon (feather class)</label>
                <input type="text" id="bf_icon" class="form-control" placeholder="e.g. feather-tablet">
                <div class="field-error" id="iconError"></div>
            </div>

            <div class="mb-3">
                <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" id="bf_requires_shipping">
                    <label class="form-check-label fw-semibold" for="bf_requires_shipping">Requires Shipping</label>
                </div>
            </div>

            <div class="mb-4">
                <label class="form-label fw-semibold">Status</label>
                <select id="bf_status" class="form-control select2-field" data-placeholder="Select status…" data-minimum-results-for-search="Infinity">
            ...
            <div class="d-flex gap-2">
                ...
                <button type="submit" class="btn btn-primary flex-fill" id="submitBtn">
                    <span class="spinner-border spinner-border-sm me-2 d-none" id="bfSpinner"></span>
                    <i class="feather-save me-2" id="bfBtnIcon"></i>
                    <span id="bfBtnText">Save</span>
                </button>
            </div>
        </form>
    </div>
...
<div class="page-header">
    <div class="page-header-left d-flex align-items-center">
        <div class="page-header-title"><h5 class="m-b-10">Book Formats</h5></div>
        <ul class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Home</a></li>
            <li class="breadcrumb-item">Bookstore</li>
            <li class="breadcrumb-item">Book Formats</li>
        </ul>
    </div>
    <div class="page-header-right ms-auto">
        <button class="btn btn-primary" onclick="openAdd()"><i class="feather-plus me-2"></i>Add Book Format</button>
    </div>
</div>
...
<div class="main-content">
    ...
    <div class="card stretch stretch-full">
        <div class="card-header d-flex align-items-center justify-content-between gap-3 py-3 flex-wrap">
            <span class="fw-semibold">All Book Formats</span>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover">
                    <thead>
                        <tr>
                            <th>#</th><th>Code</th><th>Name</th><th>Icon</th><th>Ships?</th>
                            <th class="text-center">Status</th><th class="text-end">Actions</th>
                        </tr>
                    </thead>
                    <tbody id="bfTableBody">
                        @forelse($bookFormats as $bf)
                        <tr class="bf-row" id="bf-row-{{ $bf->id }}"
                            data-status="{{ $bf->status ? 'active' : 'inactive' }}"
                            data-id="{{ $bf->id }}"
                            data-code="{{ $bf->code }}"
                            data-name="{{ $bf->name }}"
                            data-icon="{{ $bf->icon }}"
                            data-requires_shipping="{{ $bf->requires_shipping ? '1' : '0' }}"
                            data-is_active="{{ $bf->status ? '1' : '0' }}">
                            <td class="text-muted fs-12 row-index">{{ $loop->iteration }}</td>
                            <td><span class="badge bg-light-primary text-primary">{{ $bf->code }}</span></td>
                            <td class="fw-semibold fs-13 text-dark">{{ $bf->name }}</td>
                            <td class="fs-13"><i class="{{ $bf->icon }}"></i></td>
                            <td class="fs-13">{{ $bf->requires_shipping ? 'Yes' : 'No' }}</td>
                            <td class="text-center">
                                <div class="form-check form-switch d-flex justify-content-center mb-0">
                                    <input class="form-check-input status-toggle" type="checkbox"
                                           {{ $bf->status ? 'checked' : '' }} data-id="{{ $bf->id }}">
                                </div>
                            </td>
                            <td>
                                <div class="hstack gap-2 justify-content-end">
                                    <a href="javascript:void(0);" class="avatar-text avatar-md" title="Edit"
                                       onclick="openEdit(this)" data-row="bf-row-{{ $bf->id }}">
                                        <i class="feather feather-edit-3"></i>
                                    </a>
                                    <a href="javascript:void(0);" class="avatar-text avatar-md text-danger" title="Delete"
                                       onclick="deleteRow({{ $bf->id }}, '{{ addslashes($bf->name) }}')">
...
(function () {
    ...
    window.__bfPaginationRender = render;
    render();
})();
...
function resetForm(){
    ...
    document.getElementById('editBfId').value='';
    $('#bf_status').val('1').trigger('change');
    clearErrors();
}
function openAdd(){
    editMode=false; resetForm();
    document.getElementById('canvasIcon').className='feather-plus-circle me-2 text-primary';
    document.getElementById('canvasTitle').textContent='Add Book Format';
    document.getElementById('bfBtnText').textContent='Save';
    new bootstrap.Offcanvas(document.getElementById('bfCanvas')).show();
}
function openEdit(btn){
    ...
    document.getElementById('canvasTitle').textContent='Edit Book Format';
    document.getElementById('bfBtnText').textContent='Update';
    document.getElementById('editBfId').value=d.id;
    document.getElementById('bf_code').value=d.code;
    document.getElementById('bf_name').value=d.name;
    document.getElementById('bf_icon').value=d.icon;
    document.getElementById('bf_requires_shipping').checked = d.requires_shipping === '1';
    $('#bf_status').val(d.is_active).trigger('change');
    new bootstrap.Offcanvas(document.getElementById('bfCanvas')).show();
}

document.getElementById('bfForm').addEventListener('submit', function(e){
    e.preventDefault(); clearErrors();
    const id=document.getElementById('editBfId').value;
    const btn=document.getElementById('submitBtn'), sp=document.getElementById('bfSpinner'),
          ic=document.getElementById('bfBtnIcon'), tx=document.getElementById('bfBtnText');
    ...
    const payload = {
        ...
        icon: document.getElementById('bf_icon').value,
        requires_shipping: document.getElementById('bf_requires_shipping').checked ? 1 : 0,
        status: $('#bf_status').val(),
        _token: CSRF,
    };
    const url = editMode ? `/book-formats/${id}` : "{{ route('book-formats.store') }}";
    ...
    .then(({d})=>{
        ...
        if(d.status==='success'){
            ...
            bootstrap.Offcanvas.getInstance(document.getElementById('bfCanvas'))?.hide();
            setTimeout(()=>location.reload(),1200);
        } else if(d.errors){
            Object.keys(d.errors).forEach(f=>fieldErr('bf_'+f, f+'Error', d.errors[f][0]));
...
document.querySelectorAll('.status-toggle').forEach(toggle=>{
    toggle.addEventListener('change', function(){
        ...
        fetch(`/book-formats/${id}/toggle`, {method:'PATCH', headers:{'X-CSRF-TOKEN':CSRF,'Accept':'application/json'}})
        .then(r=>r.json())
...
function deleteRow(id,name){
    ...
    fetch(`/book-formats/${id}`, {method:'DELETE', headers:{'X-CSRF-TOKEN':CSRF,'Accept':'application/json'}})
    .then(r=>r.json())
    .then(d=>{
        if(d.status==='success'){
            document.getElementById(`bf-row-${id}`)?.remove();
            showToast(d.message,'success');
            window.__bfPaginationRender && window.__bfPaginationRender();
