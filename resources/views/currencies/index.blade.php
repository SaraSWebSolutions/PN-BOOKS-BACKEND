...
<div class="offcanvas offcanvas-end" tabindex="-1" id="curCanvas" style="width:420px;">
    <div class="offcanvas-header border-bottom py-3">
        <div>
            <h5 class="offcanvas-title fw-bold mb-0">
                <i id="canvasIcon" class="feather-plus-circle me-2 text-primary"></i>
                <span id="canvasTitle">Add Currency</span>
            </h5>
            <small class="text-muted">Used for pricing books and orders.</small>
        </div>
        <button type="button" class="btn-close" data-bs-dismiss="offcanvas"></button>
    </div>

    <div class="offcanvas-body">
        <form id="curForm" novalidate>
            @csrf
            <input type="hidden" id="editCurId">

            <div class="mb-3">
                <label class="form-label fw-semibold">Country</label>
                <select id="cur_country_id" class="form-control select2-field" data-placeholder="Select country…">
                    <option value=""></option>
            ...
            <div class="mb-3">
                ...
                <input type="text" id="cur_code" class="form-control text-uppercase" maxlength="10" placeholder="e.g. USD, SGD">
                <div class="field-error" id="codeError"></div>
            </div>

            <div class="mb-3">
                <label class="form-label fw-semibold">Currency Name <span class="text-danger">*</span></label>
                <input type="text" id="cur_name" class="form-control" placeholder="e.g. US Dollar">
                <div class="field-error" id="nameError"></div>
            </div>

            <div class="mb-3">
                <label class="form-label fw-semibold">Symbol <span class="text-danger">*</span></label>
                <input type="text" id="cur_symbol" class="form-control" maxlength="10" placeholder="e.g. $">
                <div class="field-error" id="symbolError"></div>
            </div>

            <div class="mb-4">
                <label class="form-label fw-semibold">Status</label>
                <select id="cur_status" class="form-control select2-field" data-placeholder="Select status…" data-minimum-results-for-search="Infinity">
            ...
            <div class="d-flex gap-2">
                ...
                <button type="submit" class="btn btn-primary flex-fill" id="submitBtn">
                    <span class="spinner-border spinner-border-sm me-2 d-none" id="curSpinner"></span>
                    <i class="feather-save me-2" id="curBtnIcon"></i>
                    <span id="curBtnText">Save</span>
                </button>
            </div>
        </form>
    </div>
...
<div class="page-header">
    ...
    <div class="page-header-right ms-auto">
        <button class="btn btn-primary" onclick="openAdd()"><i class="feather-plus me-2"></i>Add Currency</button>
    </div>
</div>
...
<div class="main-content">
    ...
    <div class="card stretch stretch-full">
        <div class="card-header d-flex align-items-center justify-content-between gap-3 py-3 flex-wrap">
            <span class="fw-semibold">All Currencies</span>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover">
                    <thead>
                        <tr>
                            <th>#</th><th>Code</th><th>Name</th><th>Symbol</th><th>Country</th>
                            <th class="text-center">Status</th><th class="text-end">Actions</th>
                        </tr>
                    </thead>
                    <tbody id="curTableBody">
                        @forelse($currencies as $cur)
                        <tr class="cur-row" id="cur-row-{{ $cur->id }}"
                            data-status="{{ $cur->status ? 'active' : 'inactive' }}"
                            data-id="{{ $cur->id }}"
                            data-code="{{ $cur->code }}"
                            data-name="{{ $cur->name }}"
                            data-symbol="{{ $cur->symbol }}"
                            data-country-id="{{ $cur->country_id }}"
                            data-is_active="{{ $cur->status ? '1' : '0' }}">
                            <td class="text-muted fs-12 row-index">{{ $loop->iteration }}</td>
                            <td><span class="badge bg-light-primary text-primary">{{ $cur->code }}</span></td>
                            <td class="fw-semibold fs-13 text-dark">{{ $cur->name }}</td>
                            <td class="fs-13">{{ $cur->symbol }}</td>
                            <td class="fs-13">{{ $cur->country->name ?? '—' }}</td>
                            <td class="text-center">
                                <div class="form-check form-switch d-flex justify-content-center mb-0">
                                    <input class="form-check-input status-toggle" type="checkbox"
                                           {{ $cur->status ? 'checked' : '' }} data-id="{{ $cur->id }}">
                                </div>
                            </td>
                            <td>
                                <div class="hstack gap-2 justify-content-end">
                                    <a href="javascript:void(0);" class="avatar-text avatar-md" title="Edit"
                                       onclick="openEdit(this)" data-row="cur-row-{{ $cur->id }}">
                                        <i class="feather feather-edit-3"></i>
                                    </a>
                                    <a href="javascript:void(0);" class="avatar-text avatar-md text-danger" title="Delete"
                                       onclick="deleteRow({{ $cur->id }}, '{{ addslashes($cur->name) }}')">
...
const ROUTES = {
    store:   "{{ route('currencies.store') }}",
    update:  "{{ route('currencies.update', ':id') }}",
    toggle:  "{{ route('currencies.toggle', ':id') }}",
    destroy: "{{ route('currencies.destroy', ':id') }}",
};

$(function(){
    $('#cur_country_id').select2({
        dropdownParent: $('#curCanvas'),
        width: '100%',
        placeholder: $('#cur_country_id').data('placeholder'),
        allowClear: true,
    });
    $('#cur_status').select2({
        dropdownParent: $('#curCanvas'),
...
(function () {
    ...
    window.__curPaginationRender = render;
    render();
})();
...
function resetForm(){
    ...
    document.getElementById('editCurId').value='';
    $('#cur_country_id').val(null).trigger('change');
    $('#cur_status').val('1').trigger('change');
    clearErrors();
}
function openAdd(){
    editMode=false; resetForm();
    document.getElementById('canvasIcon').className='feather-plus-circle me-2 text-primary';
    document.getElementById('canvasTitle').textContent='Add Currency';
    document.getElementById('curBtnText').textContent='Save';
    new bootstrap.Offcanvas(document.getElementById('curCanvas')).show();
}
function openEdit(btn){
    ...
    document.getElementById('canvasTitle').textContent='Edit Currency';
    document.getElementById('curBtnText').textContent='Update';
    document.getElementById('editCurId').value=d.id;
    document.getElementById('cur_code').value=d.code;
    document.getElementById('cur_name').value=d.name;
    document.getElementById('cur_symbol').value=d.symbol;
    $('#cur_country_id').val(d.countryId || null).trigger('change');
    $('#cur_status').val(d.is_active).trigger('change');
    new bootstrap.Offcanvas(document.getElementById('curCanvas')).show();
}

document.getElementById('curForm').addEventListener('submit', function(e){
    e.preventDefault(); clearErrors();
    const id=document.getElementById('editCurId').value;
    const btn=document.getElementById('submitBtn'), sp=document.getElementById('curSpinner'),
          ic=document.getElementById('curBtnIcon'), tx=document.getElementById('curBtnText');
    btn.disabled=true; sp.classList.remove('d-none'); ic.classList.add('d-none'); tx.textContent='Saving…';

    const payload = {
        country_id: $('#cur_country_id').val(),
        code: document.getElementById('cur_code').value,
        name: document.getElementById('cur_name').value,
        symbol: document.getElementById('cur_symbol').value,
    ...
    .then(({d})=>{
        ...
        if(d.status==='success'){
            ...
            bootstrap.Offcanvas.getInstance(document.getElementById('curCanvas'))?.hide();
            setTimeout(()=>location.reload(),1200);
        } else if(d.errors){
            const map = { country_id: 'country_idError' };
            Object.keys(d.errors).forEach(f=>fieldErr('cur_'+f, map[f] || f+'Error', d.errors[f][0]));
...
function deleteRow(id,name){
    ...
    .then(d=>{
        if(d.status==='success'){
            document.getElementById(`cur-row-${id}`)?.remove();
            showToast(d.message,'success');
            window.__curPaginationRender && window.__curPaginationRender();
