...
<div class="offcanvas offcanvas-end" tabindex="-1" id="seriesCanvas" style="width:460px;">
    <div class="offcanvas-header border-bottom py-3">
        <div>
            <h5 class="offcanvas-title fw-bold mb-0">
                <i id="canvasIcon" class="feather-plus-circle me-2 text-primary"></i>
                <span id="canvasTitle">Add Series</span>
            </h5>
            <small class="text-muted">Book series, e.g. "Harry Potter".</small>
        </div>
        <button type="button" class="btn-close" data-bs-dismiss="offcanvas"></button>
    </div>

    <div class="offcanvas-body">
        <form id="seriesForm" novalidate enctype="multipart/form-data">
            @csrf
            <input type="hidden" id="editSeriesId">

            <div class="mb-3">
                <label class="form-label fw-semibold">Series Name <span class="text-danger">*</span></label>
                <input type="text" id="series_name" class="form-control" placeholder="e.g. Harry Potter…">
                <div class="field-error" id="nameError"></div>
            </div>

            <div class="mb-3">
                <label class="form-label fw-semibold">Publisher</label>
                <select id="series_publisher_id" class="form-control">
            ...
            <div class="mb-3">
                ...
                <textarea id="series_description" class="form-control" rows="3"></textarea>
                <div class="field-error" id="descriptionError"></div>
            </div>

            <div class="mb-3">
                <label class="form-label fw-semibold">Cover Image</label>
                <input type="file" id="series_cover_image" class="form-control" accept="image/*">
                <div class="field-error" id="cover_imageError"></div>
            </div>

            <div class="mb-4">
                <label class="form-label fw-semibold">Status</label>
                <select id="series_is_active" class="form-control">
            ...
            <div class="d-flex gap-2">
                ...
                <button type="submit" class="btn btn-primary flex-fill" id="submitBtn">
                    <span class="spinner-border spinner-border-sm me-2 d-none" id="seriesSpinner"></span>
                    <i class="feather-save me-2" id="seriesBtnIcon"></i>
                    <span id="seriesBtnText">Save</span>
                </button>
            </div>
        </form>
    </div>
...
<div class="page-header">
    ...
    <div class="page-header-right ms-auto">
        <button class="btn btn-primary" onclick="openAdd()"><i class="feather-plus me-2"></i>Add Series</button>
    </div>
</div>
...
<div class="main-content">
    <div class="row g-3 mb-4">
        @foreach([
            ['label'=>'Total','value'=>$stats['total'],'icon'=>'feather-bookmark','bg'=>'rgba(99,102,241,.1)','color'=>'#6366f1'],
    ...
    <div class="card stretch stretch-full">
        <div class="card-header d-flex align-items-center justify-content-between gap-3 py-3 flex-wrap">
            <span class="fw-semibold">All Series</span>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover">
                    <thead>
                        <tr>
                            <th>#</th><th>Series</th><th>Publisher</th>
                            <th class="text-center">Books</th><th class="text-center">Status</th><th class="text-end">Actions</th>
                        </tr>
                    </thead>
                    <tbody id="seriesTableBody">
                        @forelse($series as $s)
                        <tr class="series-row" id="series-row-{{ $s->id }}"
                            data-status="{{ $s->is_active ? 'active' : 'inactive' }}"
                            data-id="{{ $s->id }}"
                            data-publisher_id="{{ $s->publisher_id }}"
                            data-name="{{ $s->name }}"
                            data-description="{{ $s->description }}"
                            data-is_active="{{ $s->is_active ? '1' : '0' }}">
                            <td class="text-muted fs-12 row-index">{{ $loop->iteration }}</td>
                            <td class="fw-semibold fs-13 text-dark">
                                <div class="d-flex align-items-center gap-2">
                                    @if($s->cover_image)
                                        <img src="{{ asset($s->cover_image) }}" style="width:32px;height:32px;border-radius:6px;object-fit:cover;">
                                    @endif
                                    {{ $s->name }}
                                </div>
                            </td>
                            <td class="text-muted fs-12">{{ $s->publisher->name ?? '—' }}</td>
                            <td class="text-center">{{ $s->books_count }}</td>
                            <td class="text-center">
                                <div class="form-check form-switch d-flex justify-content-center mb-0">
                                    <input class="form-check-input status-toggle" type="checkbox"
                                           {{ $s->is_active ? 'checked' : '' }} data-id="{{ $s->id }}">
                                </div>
                            </td>
                            <td>
                                <div class="hstack gap-2 justify-content-end">
                                    <a href="javascript:void(0);" class="avatar-text avatar-md" title="Edit"
                                       onclick="openEdit(this)" data-row="series-row-{{ $s->id }}">
                                        <i class="feather feather-edit-3"></i>
                                    </a>
                                    <a href="javascript:void(0);" class="avatar-text avatar-md text-danger" title="Delete"
                                       onclick="deleteRow({{ $s->id }}, '{{ addslashes($s->name) }}')">
                        ...
                        <tr id="noDataRow">
                            <td colspan="6" class="text-center py-5 text-muted">
                                <i class="feather-bookmark fs-30 d-block mb-2 opacity-50"></i>
                                No series found.
                            </td>
                        </tr>
                        @endforelse
...
const ROUTES = {
    ...
    update:  "{{ route('series.update', ':id') }}",
    toggle:  "{{ route('series.toggle', ':id') }}",
    destroy: "{{ route('series.destroy', ':id') }}",
};

/* ===== Front-end pagination (no reload, no controller changes) ===== */
(function () {
    const rowSelector   = '.series-row';
    ...
    window.__seriesPaginationRender = render;
    render();
})();
...
function openAdd(){
    ...
    document.getElementById('canvasTitle').textContent='Add Series';
    document.getElementById('seriesBtnText').textContent='Save';
    new bootstrap.Offcanvas(document.getElementById('seriesCanvas')).show();
}
function openEdit(btn){
    ...
    document.getElementById('canvasTitle').textContent='Edit Series';
    document.getElementById('seriesBtnText').textContent='Update';
    document.getElementById('editSeriesId').value=d.id;
    document.getElementById('series_publisher_id').value=d.publisher_id||'';
    document.getElementById('series_name').value=d.name;
    document.getElementById('series_description').value=d.description||'';
    document.getElementById('series_is_active').value=d.is_active;
    new bootstrap.Offcanvas(document.getElementById('seriesCanvas')).show();
}
...
document.getElementById('seriesForm').addEventListener('submit', function(e){
    ...
    const btn=document.getElementById('submitBtn'), sp=document.getElementById('seriesSpinner'),
          ic=document.getElementById('seriesBtnIcon'), tx=document.getElementById('seriesBtnText');
    btn.disabled=true; sp.classList.remove('d-none'); ic.classList.add('d-none'); tx.textContent='Saving…';

    const fd = new FormData();
    fd.append('name', document.getElementById('series_name').value);
    fd.append('publisher_id', document.getElementById('series_publisher_id').value);
    fd.append('description', document.getElementById('series_description').value);
    fd.append('is_active', document.getElementById('series_is_active').value);
    fd.append('_token', CSRF);
    const coverFile = document.getElementById('series_cover_image').files[0];
    ...
    .then(({d})=>{
        ...
        if(d.status==='success'){
            ...
            bootstrap.Offcanvas.getInstance(document.getElementById('seriesCanvas'))?.hide();
            setTimeout(()=>location.reload(),1200);
        } else if(d.errors){
            Object.keys(d.errors).forEach(f=>fieldErr('series_'+f, f+'Error', d.errors[f][0]));
...
function deleteRow(id,name){
    ...
    .then(d=>{
        if(d.status==='success'){
            document.getElementById(`series-row-${id}`)?.remove();
            showToast(d.message,'success');
            window.__seriesPaginationRender && window.__seriesPaginationRender();
