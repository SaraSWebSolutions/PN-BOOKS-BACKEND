...
<div class="page-header">
    <div class="page-header-left d-flex align-items-center">
        <div class="page-header-title">
            <h5 class="m-b-10">Author Management</h5>
        </div>
        <ul class="breadcrumb">
            ...
            <div class="card stretch stretch-full">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover" id="authorTable">
                            ...
                            <tbody id="authorTableBody">
                                ...
                                <tr class="single-item author-row"
                                    id="author-row-{{ $author->id }}"
                                    data-status="{{ $author->status }}">
                                    <td>
                                        <a href="{{ route('authors.show', $author) }}" class="hstack gap-3">
                                            ...
                                            <span class="text-truncate-1-line fw-semibold d-block">{{ $author->name }}</span>
                                        </a>
                                    </td>
                                    <td>{{ $author->email }}</td>
                                    <td>{{ $author->authorProfile->pen_name ?? '—' }}</td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="hstack gap-2 justify-content-end">
                                            <a href="{{ route('authors.show', $author) }}" class="avatar-text avatar-md" title="View">
                                                <i class="feather feather-eye"></i>
                                            </a>
                                            @can('edit users')
                                            <a href="{{ route('authors.edit', $author) }}" class="avatar-text avatar-md" title="Edit">
                                                <i class="feather feather-edit-3"></i>
                                            </a>
                                            @endcan
                                            @can('delete users')
                                            <a href="javascript:void(0);" class="avatar-text avatar-md text-danger"
                                               onclick="deleteAuthor({{ $author->id }})" title="Delete">
                                ...
                                <tr id="noDataRow">
                                    <td colspan="7" class="text-center py-5 text-muted">
                                        <i class="feather-edit-3 fs-30 d-block mb-2"></i>
                                        No authors found.
                                    </td>
                                </tr>
                                @endforelse
                ...
                <div class="card-footer d-flex align-items-center justify-content-between py-3">
                    <div class="text-muted fs-12" id="paginationInfo">
                        ...
                        of <span id="totalRows">{{ $authors->count() }}</span> entries
                    </div>
...
function deleteAuthor(id) {
    ...
    .then(data => {
        if (data.status === 'success') {
            document.getElementById(`author-row-${id}`)?.remove();
...
let currentPage = 1;
function getAllRows() { return Array.from(document.querySelectorAll('.author-row')); }
