...
<div class="offcanvas offcanvas-end" tabindex="-1" id="languageCanvas" style="width:420px;">
    <div class="offcanvas-header border-bottom py-3">
        <div>
            <h5 class="offcanvas-title fw-bold mb-0">
                <i id="canvasIcon" class="feather-plus-circle me-2 text-primary"></i>
                <span id="canvasTitle">Add Language</span>
            </h5>
            <small class="text-muted">Used to tag which language a book is available in.</small>
        </div>
    ...
    <div class="offcanvas-body">
        <form id="languageForm" novalidate>
            ...
            <div class="mb-3">
                ...
                <input type="text" id="language_name" class="form-control" placeholder="e.g. Tamil">
                <div class="field-error" id="nameError"></div>
            </div>

            <div class="mb-3">
                <label class="form-label fw-semibold">Native Name</label>
                <input type="text" id="language_native_name" class="form-control" placeholder="e.g. தமிழ்">
                <div class="field-error" id="native_nameError"></div>
            </div>

            <div class="mb-3">
                <label class="form-label fw-semibold">Language Code <span class="text-danger">*</span></label>
                <input type="text" id="language_code" class="form-control" maxlength="5" placeholder="e.g. ta">
                <div class="field-error" id="codeError"></div>
            </div>

            <div class="mb-4">
                <label class="form-label fw-semibold">Status</label>
                <select id="language_status" class="form-control select2-field" data-placeholder="Select status…" data-minimum-results-for-search="Infinity">
            ...
            <div class="d-flex gap-2">
                ...
                <button type="submit" class="btn btn-primary flex-fill" id="submitBtn">
                    <span class="spinner-border spinner-border-sm me-2 d-none" id="languageSpinner"></span>
                    <i class="feather-save me-2" id="languageBtnIcon"></i>
                    <span id="languageBtnText">Save</span>
                </button>
            </div>
        </form>
    </div>
...
<div class="page-header">
    <div class="page-header-left d-flex align-items-center">
        <div class="page-header-title"><h5 class="m-b-10">Languages</h5></div>
        <ul class="breadcrumb">
    ...
    <div class="page-header-right ms-auto">
        <button class="btn btn-primary" onclick="openAdd()"><i class="feather-plus me-2"></i>Add Language</button>
    </div>
</div>
...
<div class="main-content">
    <div class="row g-3 mb-4">
        @foreach([
            ['label'=>'Total','value'=>$stats['total'],'icon'=>'feather-message-square','bg'=>'rgba(99,102,241,.1)','color'=>'#6366f1'],
    ...
    <div class="card stretch stretch-full">
        <div class="card-header d-flex align-items-center justify-content-between gap-3 py-3 flex-wrap">
            <span class="fw-semibold">All Languages</span>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover">
                    <thead>
                        <tr>
                            <th>#</th><th>Name</th><th>Native Name</th><th>Code</th>
                            <th class="text-center">Status</th><th class="text-end">Actions</th>
                        </tr>
                    </thead>
                    <tbody id="languageTableBody">
                        @forelse($languages as $language)
                        <tr class="language-row" id="language-row-{{ $language->id }}"
                            data-status="{{ $language->status ? 'active' : 'inactive' }}"
                            data-id="{{ $language->id }}"
                            data-name="{{ $language->name }}"
                            data-native_name="{{ $language->native_name }}"
                            data-code="{{ $language->code }}"
                            data-is_active="{{ $language->status ? '1' : '0' }}">
                            <td class="text-muted fs-12 row-index">{{ $loop->iteration }}</td>
                            <td class="fw-semibold fs-13 text-dark">{{ $language->name }}</td>
                            <td class="fs-13">{{ $language->native_name ?: '—' }}</td>
                            <td><span class="badge bg-light-primary text-primary">{{ strtoupper($language->code) }}</span></td>
                            ...
                            <td>
                                <div class="hstack gap-2 justify-content-end">
                                    <a href="javascript:void(0);" class="avatar-text avatar-md" title="Edit"
                                       onclick="openEdit(this)" data-row="language-row-{{ $language->id }}">
                                        <i class="feather feather-edit-3"></i>
                                    </a>
                                    <a href="javascript:void(0);" class="avatar-text avatar-md text-danger" title="Delete"
                                       onclick="deleteRow({{ $language->id }}, '{{ addslashes($language->name) }}')">
                        ...
                        <tr id="noDataRow">
                            <td colspan="6" class="text-center py-5 text-muted">
                                <i class="feather-message-square fs-30 d-block mb-2 opacity-50"></i>
                                No languages found.
                            </td>
                        </tr>
                        @endforelse
...
const ROUTES = {
    ...
    update:  "{{ route('languages.update', ':id') }}",
    toggle:  "{{ route('languages.toggle', ':id') }}",
    destroy: "{{ route('languages.destroy', ':id') }}",
};

$(function(){
    $('#language_status').select2({
        dropdownParent: $('#languageCanvas'),
        width: '100%',
...
(function () {
    const rowSelector   = '.language-row';
    ...
    window.__languagePaginationRender = render;
    render();
})();
...
function resetForm(){
    ...
    $('#language_status').val('1').trigger('change');
    clearErrors();
}
function openAdd(){
    editMode=false; resetForm();
    document.getElementById('canvasIcon').className='feather-plus-circle me-2 text-primary';
    document.getElementById('canvasTitle').textContent='Add Language';
    document.getElementById('languageBtnText').textContent='Save';
    new bootstrap.Offcanvas(document.getElementById('languageCanvas')).show();
}
function openEdit(btn){
    ...
    document.getElementById('canvasTitle').textContent='Edit Language';
    document.getElementById('languageBtnText').textContent='Update';
    document.getElementById('editLanguageId').value=d.id;
    document.getElementById('language_name').value=d.name;
    document.getElementById('language_native_name').value=d.native_name || '';
    document.getElementById('language_code').value=d.code;
    $('#language_status').val(d.is_active).trigger('change');
    new bootstrap.Offcanvas(document.getElementById('languageCanvas')).show();
}
...
document.getElementById('languageForm').addEventListener('submit', function(e){
    ...
    const btn=document.getElementById('submitBtn'), sp=document.getElementById('languageSpinner'),
          ic=document.getElementById('languageBtnIcon'), tx=document.getElementById('languageBtnText');
    btn.disabled=true; sp.classList.remove('d-none'); ic.classList.add('d-none'); tx.textContent='Saving…';

    const payload = {
        name: document.getElementById('language_name').value,
        native_name: document.getElementById('language_native_name').value,
        code: document.getElementById('language_code').value,
        status: $('#language_status').val(),
        _token: CSRF,
    };
    ...
    .then(({d})=>{
        ...
        if(d.status==='success'){
            ...
            bootstrap.Offcanvas.getInstance(document.getElementById('languageCanvas'))?.hide();
            setTimeout(()=>location.reload(),1200);
        } else if(d.errors){
            Object.keys(d.errors).forEach(f=>fieldErr('language_'+f, f+'Error', d.errors[f][0]));
...
function deleteRow(id,name){
    ...
    .then(d=>{
        if(d.status==='success'){
            document.getElementById(`language-row-${id}`)?.remove();
            showToast(d.message,'success');
            window.__languagePaginationRender && window.__languagePaginationRender();
